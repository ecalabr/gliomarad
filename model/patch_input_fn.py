...
def _load_multicon_and_labels(study_dir, feature_prefx, label_prefx, data_fmt, out_dims):
    """
...
def _load_roi_multicon_and_labels(study_dir, feature_prefx, label_prefx, mask_prefx, patch_size, aug=True, interp=1):
    """
    ...
    :param patch_size: (int) The patch size in pixels (patches are square)
...
def _tf_patches(data, labels, patch_size, chan_dim, data_format):
    ...
    data = tf.reshape(data, [-1, patch_size, patch_size, chan_dim])
    labels = tf.extract_image_patches(labels, ksizes=ksizes, strides=strides, rates=rates, padding='SAME')
    labels = tf.reshape(labels, [-1, patch_size, patch_size, 1])
