# -*- coding: utf-8 -*-
# Implementation of Wang et al 2017: Automatic Brain Tumor Segmentation using Cascaded Anisotropic Convolutional Neural
...
def test(dir_list, bias=True):
    ...
    for i, path in enumerate(data_nm):
        print(str(i) + ': ' + os.path.join(data_rt, path) + " >> " + os.path.join(save_dr[i],
                                                                                  idno[i] + "_tumor_seg.nii.gz"))
    ...
    config = {"data": {"data_root": data_rt,
              ...
              "network1ax": {"net_type": "MSNet",
                             ...
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_wt32_20000.ckpt")},
              "network1sg": {"net_type": "MSNet",
                             "net_name": "MSNet_WT32sg",
                             "downsample_twice": True,
                             "data_shape": [19, 160, 180, 4],
                             "label_shape": [11, 160, 180, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_wt32sg_20000.ckpt")},
              "network1cr": {"net_type": "MSNet",
                             "net_name": "MSNet_WT32cr",
                             "downsample_twice": True,
                             "data_shape": [19, 160, 160, 4],
                             "label_shape": [11, 160, 160, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_wt32cr_20000.ckpt")},

              "network2ax": {"net_type": "MSNet",
                             "net_name": "MSNet_TC32",
                             "downsample_twice": True,
                             "data_shape": [19, 96, 96, 4],
                             "label_shape": [11, 96, 96, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_tc32_20000.ckpt")},
              "network2sg": {"net_type": "MSNet",
                             "net_name": "MSNet_TC32sg",
                             "downsample_twice": True,
                             "data_shape": [19, 96, 96, 4],
                             "label_shape": [11, 96, 96, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_tc32sg_20000.ckpt")},
              "network2cr": {"net_type": "MSNet",
                             "net_name": "MSNet_TC32cr",
                             "downsample_twice": True,
                             "data_shape": [19, 96, 96, 4],
                             "label_shape": [11, 96, 96, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_tc32cr_20000.ckpt")},

              "network3ax": {"net_type": "MSNet",
                             "net_name": "MSNet_EN32",
                             "downsample_twice": False,
                             "data_shape": [19, 96, 96, 4],
                             "label_shape": [11, 96, 96, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_en32_20000.ckpt")},
              "network3sg": {"net_type": "MSNet",
                             "net_name": "MSNet_EN32sg",
                             "downsample_twice": False,
                             "data_shape": [19, 96, 96, 4],
                             "label_shape": [11, 96, 96, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_en32sg_20000.ckpt")},
              "network3cr": {"net_type": "MSNet",
                             "net_name": "MSNet_EN32cr",
                             "downsample_twice": False,
                             "data_shape": [19, 96, 96, 4],
                             "label_shape": [11, 96, 96, 1],
                             "class_num": 2,
                             "model_file": os.path.join(sc_p, "model17/msnet_en32cr_20000.ckpt")},

              "testing": {"test_slice_direction": "all",
    ...
    save_dir = config_data['save_folder']
    test_time = []
    ...
    for i in range(image_num):
        ...
        if config_test.get('whole_tumor_only', False) is True:
            ...
            label2 = set_ND_volume_roi_with_bounding_box_range(label2, bbox1[0], bbox1[1], pred2)  # typo corrected here
    ...
    # np.savetxt(save_dir + '/test_time.txt', test_time)
    sess.close()
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        ...
        print('    python test.py /path/to/data_directory')
        exit()
